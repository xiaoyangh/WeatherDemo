...
- (void)setNowFrame:(NowFrame *)nowFrame
{
    ...
    self.updateTimeLable.frame = nowFrame.updateLableFrame;
      ...
      NSString *tempStr=[NSString stringWithFormat:@"%.1f",[weatherData.now.tmp floatValue]];
    // 温度标签
    self.tmp.text = tempStr;
    ...
    // 体感温度
    NSString *fl = [NSString stringWithFormat:@"speed %@",weatherData.now.wind.sc];
    self.fl.text = fl;
    self.fl.frame = nowFrame.flFrame;
//    self.fl.hidden=YES;
    
    // 风向风力及湿度
    NSString *windhum  = [NSString stringWithFormat:@"humidity %@",weatherData.now.hum];
