...
//    NSString *tmp = weatherData.now.tmp;
    NSString *tmp=[NSString stringWithFormat:@"%.1f",[weatherData.now.tmp floatValue]];
    ...
    // 体感温度
    NSString *fl = [NSString stringWithFormat:@"humidity %@",weatherData.now.fl];
    CGSize flSize = [fl sizeWithAttributes:comFontDict];
    CGFloat flY = CGRectGetMaxY(_tmpFrame) + 4;
    _flFrame = CGRectMake(tmpX, flY, flSize.width+10, flSize.height);
    
    // 风向风力及湿度
    NSString *windhum  = [NSString stringWithFormat:@"%@%@ humidity %@",weatherData.now.wind.dir,weatherData.now.wind.sc,weatherData.now.hum];
